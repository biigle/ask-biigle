...
class ChatController extends Controller
{
    ...
    public function chat(Request $request)
    {
        ...
        $apiKey = config('askbiigle.llm_api_key');
        $model = config('askbiigle.llm_algorithm');
        $apiUrl = config('askbiigle.llm_api_url');
        ...
        $payload = [
            'model' => $model,
            'messages' => $messages,
            'enable-tools' => (bool) config('askbiigle.llm_enable_tools'),
            'temperature' => (float) config('askbiigle.llm_temperature'),
            'top_p' => (float) config('askbiigle.llm_top_p'),
        ];

        $arcanaId = config('askbiigle.llm_arcana_id');
        if (!empty($arcanaId)) {
            $payload['arcana'] = ['id' => $arcanaId];
        }

        $maxTries = 3;
        $timeout = (int) config('askbiigle.llm_timeout', 120);
        $response = null;
        ...
        for ($attempt = 1; $attempt <= $maxTries; $attempt++) {
            try {
                $response = Http::acceptJson()
                    ...
                    ->withToken($apiKey)
                    ->withHeaders([
                        'inference-service' => config('askbiigle.llm_inference_service'),
                    ])
                    ->post($apiUrl, $payload);
